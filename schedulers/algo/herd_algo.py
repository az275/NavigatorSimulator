...
# pipeline 1 throughputs on full GPU
# 270, 45, 270, 70
# pipeline 2
# 125, 7555, 92, 4.82 (??)

def _model_affinity(m1: Model, m2: Model) -> float:
    alpha_1 = m1.exec_time_constants[24][0]
    beta_1 = m1.exec_time_constants[24][1]
    alpha_2 = m2.exec_time_constants[24][0]
    beta_2 = m2.exec_time_constants[24][1]
...
def get_herd_assignment(task_types: list[tuple[int,int]], all_models: list[Model], 
                        task_tputs: dict[tuple[int,int], float]) -> tuple[list[int], list[tuple[int,int]]]:
    """
    ...
    send_rate = 1 / DEFAULT_CREATION_INTERVAL_PERCLIENT * 1000
