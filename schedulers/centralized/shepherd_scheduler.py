...
class ShepherdScheduler(Scheduler):
    ...
    def __init__(self, simulation, herd_assignment=None):
        """
            @param worker_groups is a list of sublists representing one group of workers.
            @param task_type_to_group is a map from task type specified by (workflow id, task id) to
            the group index in [worker_groups].
            See HERD ILP for details.
        """
    ...
    def _flex_form_largest_batch(self, model_id: int, time: float) -> Batch:
        ...
        return Batch(self._batch_counter-1, tasks)
