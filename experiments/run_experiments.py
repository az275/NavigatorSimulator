...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--type",
        choices=["centralheft", "decentralheft", "hashtask", "boostdecentralheft"],
        help="Scheduler type", required=True)
    parser.add_argument("-bp", "--boost-policy", choices=["total", "remaining"],
        help="Boost policy", default="total")
    
    policy_arg_map = { "total": Job._TOTAL_JOB_TIME, "remaining": Job._REMAINING_JOB_TIME }
    ...
    folder_name = os.path.join(current_directory, "results/" + "/" + args.type + "/")
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
        print("Directory created:", folder_name)
    else:
        print("Directory already exists:", folder_name)
    OUTPUT_FILE_NAMES[args.type] = folder_name

    # 2. Run and collect data
    if args.type == "centralheft":
    ...
    elif args.type == "boostdecentralheft":
        OUTPUT_FILENAME = "boostdecentralheft"

        sim = Simulation_decentral(simulation_name="boostdecentralheft", job_split="PER_TASK",
                                    num_workers=TOTAL_NUM_OF_NODES, job_types_list=plotting_job_type_list,
                                    dynamic_adjust=False,
                                    consider_load=True, consider_cache=True,
                                    produce_breakdown=True,
                                    use_boost=True,
                                    boost_policy=policy_arg_map[args.boost_policy])
        sim.run()
        
        result_to_export = sim.result_to_export
        result_to_export.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "job_breakdown.csv")

        event_log = sim.event_log
        event_log.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "events_by_time.csv")
        
        result_to_export = sim.result_to_export
        result_to_export.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "job_breakdown.csv")

        tasks_logging_times = sim.tasks_logging_times
        tasks_logging_times.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "loadDelay_" + str(
            LOAD_INFORMATION_STALENESS) + "_placementDelay_" + str(PLACEMENT_INFORMATION_STALENESS) + ".csv")
        
        sim.batch_exec_log.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "batch_log.csv")
        
        worker_model_histories = pd.concat(list(map(lambda w: w.model_history_log, sim.workers)), 
                                           keys=list(map(lambda w: w.worker_id, sim.workers)), 
                                           names=['worker_id']).reset_index(level='worker_id')
        worker_model_histories = worker_model_histories.sort_values(by="start_time")
        worker_model_histories.to_csv(OUTPUT_FILE_NAMES["boostdecentralheft"] + "model_history_log.csv")

    elif args.type == "hashtask":
